...
const Loginform = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault()
    try {
      const response = await axios.post('https://marvelous-gentleness-production.up.railway.app/api/Authentication/login', {
        username,
        password
      });
      setErrorMsg('')
        ...
        font-extrabold text-center mb-4 text-white">Đăng Nhập</span>
          ...
          <div className="flex flex-col md:flex-row space-x-[50px] mr-4 -mt-44 md:mt-0">
            ...
            <div className="flex flex-col space-y-2 pt-2 pl-2">
              <div className="flex flex-col space-y-1 relative hover:scale-105 
            transition-transform duration-300">
                <div className="bg-transparent md:bg-white pl-6 md:pl-8 pt-8 rounded-lg">
                  <div className="relative bg-inherit">
                    <input value={username} onChange={(e) => setUsername(e.target.value)} type="text" id="username" name="username" className="peer bg-transparent
                   ...
                   px-2 ring-gray-500 focus:ring-sky-600 focus:outline-none
                    ...
                    <label for="username" className="absolute cursor-text left-0 -top-6 md:-top-3 text-sm 
              ...
              <div className="flex flex-col space-y-1 relative hover:scale-105 transition-transform
             duration-300">
                <div className="bg-transparent md:bg-white pl-6 md:pl-8 pt-8 rounded-lg">
                  <div className="relative bg-inherit">
                    <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" id="password" name="password" className="peer 
                  ...
                  focus:outline-none focus:border-rose-600" placeholder="" />
                    <label for="password" className="absolute cursor-text left-0 -top-6 md:-top-3 text-sm 
                  text-blue_steam bg-inherit mx-1 px-1 peer-placeholder-shown:text-base 
                  peer-placeholder-shown:top-2 peer-focus:-top-8 md:peer-focus:-top-3 
                  peer-placeholder-shown:text-slate-200 
                  md:peer-placeholder-shown:text-blue_steam
              ...
              <div className="flex flex-grow space-x-2 pl-7 pt-2 hover:scale-105 transition-transform duration-300">
                ...
                <label className="text-blue_steam md:text-slate-700 font-bold">Ghi nhớ tôi</label>
              </div>
              <button type='submit' className="flex cursor-pointer hover:scale-105 transition-transform duration-300 rounded-sm 
             ...
             from-blue_steam to-blue_steam_login">
              ...
              {successMsg && (
                <div className='flex flex-row space-x-2 pt-2'>
                  <span className='text-xs ml-[10%] text-green-500 font-extrabold'>{successMsg}</span>
                  <img className='w-[15px] h-[15px]' src={checked} />
                </div>
              )
              }
              {errorMsg && (
                <div className='flex flex-row space-x-2 pt-2'>
                  <span className='text-xs ml-[10%] text-red-500 font-extrabold'>{errorMsg}</span>
                  <img className='w-[15px] h-[15px]' src={unchecked} />
                </div>
              )
              }
              <div className="pl-2 md:pl-8 mx-auto py-4">
                <span className="text-xs inline-block cursor-pointer hover:underline hover:scale-105 transition-transform 
           ...
           via-blue_steam to-transparent shadow-lg"></div>
            ...
            <div className="flex flex-col pt-0 pb-5 md:pt-7  md:pr-8 pr-10">
              <label className="text-blue_steam font-semibold">Đăng nhập bằng nền tảng khác</label>
              <Otherlogin />
            </div>
            {/* 3 */}
          </div>
